...
일반 게시글 대상 리뷰 요청은 게시글 쪽 흐름만 관리한다. 다만 포트폴리오 관리에서 발행한 게시글은 `portfolio_projects.published_post_id`로 원본 프로젝트를 찾아 프로젝트 코치 상태도 함께 갱신한다.

## 7-2. 도움말 챗봇 MVP 기준

JungleLog의 첫 챗봇은 AI 상담 챗봇이 아니라 서비스 사용 흐름을 안내하는 MVP 도움말이다.

현재 기준:

- 승인 완료된 STUDENT에게만 노출한다.
- COACH, ADMIN, 승인 대기/정지/거절 사용자에게는 노출하지 않는다.
- OpenAI, RAG, Agent를 호출하지 않는다.
- 빠른 질문과 키워드 매칭으로 rule-based 답변을 반환한다.
- 포트폴리오 관리, GitHub 프로젝트 등록, 학습 기록 연결, AI 도우미, 코치 리뷰 요청, 공개/비공개 발행 차이만 안내한다.

나중에 확장할 때의 방향:

```txt
현재: helpChatbotRules.ts rule-based 응답
다음: JungleLog 기록/RAG 검색 결과를 도움말 context로 사용
다음: Agent가 질문 의도를 판단해 페이지 이동, 자료 검색, 포트폴리오 상담 tool을 선택
```
