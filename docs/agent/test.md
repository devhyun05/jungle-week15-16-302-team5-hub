...
- 학생 계정에서 `/ai-assistant` → 프로젝트 선택 → `포트폴리오 글 만들기` 또는 `면접 예상 질문 만들기` → 결과 저장까지 브라우저 기준으로 확인해야 한다.
...
- `포트폴리오 글 만들기` 또는 `면접 예상 질문 만들기` 버튼 클릭 후 결과 영역에 생성 결과가 표시되는지 확인한다.
...
frontend npm run build: success
backend compileall app: success
browser /ai-assistant unauthenticated redirect: /login
```
...
- `포트폴리오 글 만들기` 또는 `면접 예상 질문 만들기`를 누르면 새 생성 결과가 결과 영역에 표시되는지 확인한다.
...
- AI 도우미 화면에서 `일반 생성`, `RAG 기반 생성`, `Agent 기반 생성` 선택 UI가 보이지 않는지 확인한다.
- AI 도우미 화면에서 `포트폴리오 글 만들기`, `면접 예상 질문 만들기` 두 액션이 명확히 보이는지 확인한다.
- 참고 자료 영역에서 GitHub README, GitHub 커밋 메시지, 연결된 JungleLog 기록, 코치 피드백, 기존 포트폴리오 글이 AI 생성 재료로 보이는지 확인한다.
...
## 2026-06-17 QA: AI 도우미 생성 방식 UI 제거
...
- `http://localhost:5173/ai-assistant`로 접근했을 때 현재 세션은 `로그인 상태를 확인하고 있습니다.` 화면에 머물렀다.
- 이 상태에서는 AI 도우미 본문 버튼까지 시각 확인하지 못했다.
- 코드 검색과 빌드 기준으로 `일반 생성`, `RAG 기반 생성`, `Agent 기반 생성` 선택 UI 제거는 확인했다.
- 실제 학생 로그인 세션에서 아래 수동 확인 항목을 이어서 보면 된다.

### 확인할 것

- `/ai-assistant`에서 `일반 생성`, `RAG 기반 생성`, `Agent 기반 생성` 문구가 화면에 보이지 않는다.
- 프로젝트 선택 영역은 유지된다.
- `포트폴리오 글 만들기` 버튼을 눌러도 생성 API가 바로 호출되지 않고 작업 선택 상태만 바뀐다.
- `면접 예상 질문 만들기` 버튼을 눌러도 생성 API가 바로 호출되지 않고 작업 선택 상태만 바뀐다.
- `생성하기` 버튼을 눌렀을 때만 AI 생성 API가 호출된다.
- 생성 결과 저장 버튼은 기존처럼 `포트폴리오 글로 저장` 또는 `면접 질문으로 저장`으로 동작한다.
- 참고 자료 영역에 README, 커밋, JungleLog 기록, 저장된 결과, 코치 피드백 상태가 보인다.

## 2026-06-17 QA: AI 생성 실행 분리와 알림 동기화
...
- `/ai-assistant`에서 `포트폴리오 글 만들기`, `면접 예상 질문 만들기`는 active 상태만 바꾸고 OpenAI 호출을 하지 않는다.
- 프로젝트를 선택하지 않고 `생성하기`를 누르면 `프로젝트를 먼저 선택해주세요.` 안내가 보인다.
- 작업 유형을 선택하지 않고 `생성하기`를 누르면 `생성할 작업을 선택해주세요.` 안내가 보인다.
- 작업 유형을 선택한 뒤 `생성하기`를 누르면 Agent -> RAG -> direct fallback 순서로 생성이 시도된다.
- 생성 성공 후 학생 본인 알림에 AI 생성 완료 알림이 생긴다.
- 포트폴리오 게시글 발행/갱신 성공 후 학생 본인 알림에 발행/갱신 알림이 생긴다.
- 포트폴리오 프로젝트 대상으로 코치 리뷰 요청을 보내면 프로젝트 카드의 코치 상태가 `요청 대기 중`으로 바뀐다.
- 코치가 `검토 중`, `수정 요청`, `피드백 완료`로 전송하면 학생 포트폴리오 프로젝트의 코치 상태도 같은 값으로 바뀐다.
- 학생이 대기 중 리뷰 요청을 취소하면 코치에게 취소 알림이 생긴다.
- 헤더 종 아이콘에 읽지 않은 알림 수 badge가 표시된다.
- 알림을 클릭하면 읽음 처리되고 badge 수가 줄어든다.
- 알림의 `x` 버튼을 누르면 해당 알림이 목록에서 사라진다.
- 다른 사용자의 알림 id로 삭제 요청을 보내면 404가 반환되어야 한다.

### 추가 구현 필요

- 코치가 담당한 리뷰 요청 대상 게시글/포트폴리오가 새 댓글이나 수정으로 업데이트되었을 때 코치에게 알림을 보내는 흐름은 아직 별도 이벤트 훅이 없다.
- 실시간 push 알림은 아직 없고, 현재는 드롭다운을 열거나 화면을 다시 불러올 때 최신 알림을 조회한다.
