...
- 게시글 탭에서 포트폴리오 발행 게시글을 대상으로 코치 리뷰 요청을 보내도 프로젝트 카드의 코치 상태가 `요청 대기 중`으로 바뀐다.
...
## 2026-06-17 QA: 포트폴리오 게시글 리뷰 요청 상태 동기화
...
- 코치 리뷰 요청 화면에서 `게시글` 탭을 선택한다.
- `포트폴리오 관리` 카테고리의 포트폴리오 게시글을 선택해 리뷰 요청을 보낸다.
- 포트폴리오 관리 화면으로 돌아가면 해당 프로젝트 카드의 `코치` 상태가 `요청 대기 중`으로 보인다.
- 이미 요청이 생성된 상태에서도 `/portfolio`를 새로고침하면 목록 조회 보정 로직으로 상태가 맞춰진다.
- 코치가 해당 요청을 `검토 중`, `수정 요청`, `피드백 완료`로 바꾸면 프로젝트 카드 상태도 같이 바뀐다.

## 2026-06-17 QA: 학생용 도움말 챗봇 MVP
...
- 승인 완료된 STUDENT 화면 오른쪽 아래에 `도움말` floating 버튼이 보인다.
- COACH 화면에는 도움말 챗봇이 보이지 않는다.
- ADMIN 화면에는 도움말 챗봇이 보이지 않는다.
- 승인 대기/정지/거절 화면에는 도움말 챗봇이 보이지 않는다.
- 도움말 버튼을 누르면 `JungleLog 도움말` 패널이 열린다.
- 닫기 버튼 또는 floating 버튼으로 패널을 닫을 수 있다.
- 빠른 질문 버튼을 누르면 해당 rule-based 답변이 메시지 영역에 추가된다.
- 입력창에 `GitHub 프로젝트 어떻게 등록해?`처럼 입력하면 GitHub 등록 안내가 나온다.
- 입력창에 지원하지 않는 프로젝트 상담/코드 분석 질문을 입력하면 RAG/Agent 확장 예정 안내가 나온다.
- 답변 안의 이동 버튼을 누르면 해당 라우트로 이동한다.
- 챗봇 사용 중 OpenAI API 호출이 발생하지 않는다.

### 추가 구현 필요

- 실제 AI 챗봇은 아직 없다.
- RAG 기반 게시글 검색, 포트폴리오 상담, 프로젝트 기반 질문 답변은 다음 AI 고도화 단계에서 구현한다.
