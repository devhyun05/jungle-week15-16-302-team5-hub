...
- `frontend/src/app/pages/ai/AIAssistant.tsx`: AI 작업 버튼에서 백엔드 `/ai/generate`를 호출하도록 연결했다.
...
2. `포트폴리오 글 만들기` 또는 `면접 예상 질문 만들기` 버튼을 누른다.
...
- 남음: 프로젝트를 가진 학생 계정으로 브라우저에서 `포트폴리오 글 만들기` 또는 `면접 예상 질문 만들기` 버튼을 직접 눌러 저장 흐름까지 확인.
...
- `포트폴리오 글 만들기` 또는 `면접 예상 질문 만들기` 버튼을 누른다.
...
## 2026-06-17 학습 기록: AI 도우미 UI와 내부 아키텍처 분리

### 수정한 파일

- `frontend/src/app/pages/ai/AIAssistant.tsx`
  - 사용자에게 보이던 `일반 생성`, `RAG 기반 생성`, `Agent 기반 생성` 선택 UI를 제거했다.
  - `포트폴리오 글 만들기`, `면접 예상 질문 만들기` 두 액션을 생성 버튼으로 정리했다.
- `docs/agent/wei.md`
  - 일반/RAG/Agent는 사용자 선택 UI가 아니라 내부 구현 단계라는 기준을 추가했다.

### 왜 이렇게 바꿨나

사용자는 RAG나 Agent라는 구현 방식을 고르고 싶은 것이 아니라, 프로젝트를 바탕으로 결과물을 만들고 싶어 한다.

따라서 화면은 단순하게 유지하고, 내부에서 어떤 자료를 어떻게 검색하고 어떤 도구를 호출하는지는 문서와 백엔드 구조에서 설명하는 편이 좋다.

### 현재 흐름

```txt
프로젝트 선택
-> 포트폴리오 글 만들기 또는 면접 예상 질문 만들기
-> /ai/generate 호출
-> 결과 표시
-> 포트폴리오 프로젝트에 저장
```
...
```txt
초기: direct prompt context 생성
다음: 내부 RAG 검색 추가
다음: Agent가 MCP tool과 RAG 검색을 실행
```

UI 버튼은 그대로 두고 내부 구현만 고도화할 수 있다.
